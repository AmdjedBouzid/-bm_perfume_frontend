    ...
    <div
      className={`fixed bg-black text-secondary shadow-[rgba(0,0,0,0.25)] transition-all duration-300 ${
        isMobile
          ...
          : `top-0 right-0 h-screen rounded-l-2xl ${isOpen ? "w-[229px]" : "w-[75px]"}`
      }`}
      ...
      {!isMobile ? (
        <div className="flex flex-col items-end mt-20 gap-20 pr-4">
          <div className="flex flex-row-reverse items-center space-x-reverse space-x-3">
            ...
            <span className={`${isOpen ? "block" : "hidden"} text-[22px] font-medium`}>المسؤول</span>
          </div>
...
<nav className="flex flex-col space-y-6 font-semibold text-[20px] w-full">
  ...
  <Link href="/Administration/">
    <SidebarItem isOpen={isOpen} imgSrc="/icons/Products.svg" text="الإعدادات" />
          ...
          <SidebarItem isOpen={isOpen} imgSrc="/icons/logout-03.svg" text="تسجيل الخروج" customClass="mt-auto text-[20px] mt-20 font-medium mb-6" />
        </div>
      ) : (
...
const SidebarItem = ({ isOpen, Icon, imgSrc, text, customClass = "" }) => {
  return (
    <div className={`flex flex-row-reverse items-center space-x-reverse space-x-3 text-right p-2 cursor-pointer hover:bg-gray-800 rounded ${customClass}`}>
      {imgSrc ? (
